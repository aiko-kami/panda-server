...
const retrieveProjectLocation = async (req, res) => {
	try {
		...
		const projectData = await projectService.retrieveProjectByLink(projectLink, ["-_id", "projectLink", "title", "projectLink", "members", "statusInfo", "location"]);
...
const retrieveProjectStatus = async (req, res) => {
	try {
		const { projectLink = "" } = req.params;
		...
		//Retrieve project data
		const projectData = await projectService.retrieveProjectByLink(projectLink, ["-_id", "projectId", "title", "projectLink", "members", "statusInfo", "startDate", "visibility"]);
		if (projectData.status !== "success") {
			return apiResponse.serverErrorResponse(res, projectData.message);
		}
